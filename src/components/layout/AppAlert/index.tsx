...
import { getMostRecentAlert, removeAlert } from '../../../store/app';
...
const AppAlert = (): JSX.Element => {
  ...
  const alert = useSelector(getMostRecentAlert);
    ...
    >
      {alert && (
        <Alert severity={alert.severity} onClose={() => setOpen(false)} variant="filled">
          {t(alert.message, { defaultValue: alert.message, ...(alert?.messageOptions || {}) })}
        </Alert>
      )}
    </Snackbar>
  );
